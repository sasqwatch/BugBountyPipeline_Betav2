...
resource "digitalocean_droplet" "task-alienvault" {
  depends_on = [tls_private_key.temp_pvt_key]

  image = "ubuntu-20-04-x64"
  name = "task-alienvault"
  region = "nyc3"
  size = "s-1vcpu-1gb"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "wget https://github.com/maliciousgroup/Alienvault/releases/download/v1.0/alienvault.zip",
      "unzip alienvault.zip",
      "mv alienvault /usr/bin/",
...
output "web_ipv4_address" {
  ...
  value       = digitalocean_droplet.task-alienvault.ipv4_address
}