...
resource "digitalocean_droplet" "task-linkfinder" {
  depends_on = [tls_private_key.temp_pvt_key]

  image = "ubuntu-18-04-x64"
  name = "task-linkfinder"
  region = "nyc3"
  size = "s-1vcpu-1gb"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "git clone https://github.com/GerbenJavado/LinkFinder /tmp/linkfinder",
...
output "web_ipv4_address" {
  ...
  value       = digitalocean_droplet.task-linkfinder.ipv4_address
}